...
$$
\sigma_{\hat{p}_{1} - \hat{p_{2}}} = \sqrt{\frac{p_{1}q_{1}}{m} + \frac{p_{2}q_{2}}{n}} 
$$
...
$$
n = \frac{[z_{\alpha}\sqrt{(p_{1} + p_{2})(q_{1} + q_{2})/2} + z_{\beta}\sqrt{p_{1}q_{1} + p_{2}q_{2}}]^{2}}{d^{2}}
$$
...
-   Look up values in the F table.
-   Perform a hypothesis test for the equality of population variances.
-   Calculate and interpret a confidence interval for the ratio of population variances.

## The F-Distribution {.smaller}

-   In order to test hypotheses about two population variances, we need to talk about a new distribution: **the F distribution**.
-   Let $X_{1}$ and $X_{2}$ be independent chi-squared random variables with $\nu_{1}$ and $\nu_{2}$ degrees of freedom, respectively. The F distribution with $\nu_{1}$ numerator degrees of freedom and $\nu_{2}$ denominator degrees of freedom is defined to be the distribution of the ratio:

$$
F = \frac{X_{1}/\nu_{1}}{X_{2}/\nu_{2}}
$$

-   Sometimes the degrees of freedom will be listed as: $F_{\nu_{1},\nu_{2}}$.

## The F-Distribution

-   From this, we can show that if we collect a random sample of size $m$ from a $N(\mu_{1},\sigma_{1})$ distribution and an independent random sample of size $n$ from a $N(\mu_{2},\sigma_{2})$ distribution, then

$$
F_{m-1,n-1} = \frac{S_{1}^{2}/\sigma_{1}^{2}}{S_{2}^{2}/\sigma_{2}^{2}}
$$

follows a normal distribution with $m-1$ numerator degrees of freedom and $n-1$ denominator degrees of freedom.
...
-   It is a right-skewed distribution that has strictly positive values.
-   Once again, the density is too difficult to integrate, and therefore a table is required.
-   $$
    F_{p,\nu_{1},\nu_{2}} = \frac{1}{F_{1-p,\nu_{2},\nu_{1}}}
    $$

## Exercise 10.60 {.smaller}
...
g.  $P(F < 6.16)$ for $\nu_{1} = 6$, $\nu_{2} = 4$.
h.  $P(0.177 < F < 4.74)$ for $\nu_{1} = 10$, $\nu_{2} = 5$.

## Testing Hypotheses {.smaller}

-   To formally state what we have shown earlier, the confidence interval and hypotheses tests formed in this section will depend on the following.

::: callout-important
## Theorem

Let $X_{1}$, ... , $X_{m}$ be a random sample from a normal distribution with variance $\sigma_{1}^{2}$, let $Y_{1}$, ... , $Y_{n}$ be another random sample (independent of the $X_{i}$’s) from a normal distribution with variance $\sigma_{2}^{2}$, and let $S_{1}^{2}$ and $S_{2}^{2}$ denote the two sample variances. Then the rv
...
-   Under a null hypothesis of equal variances, this reduces to the ratio of sample variances.
...
|---|---|
| $H_{a}: \sigma_{1}^{2} > \sigma_{2}^{2}$ | $f \geq F_{\alpha, m-1, n-1}$ |
| $H_{a}: \sigma_{1}^{2} < \sigma_{2}^{2}$ | $f \leq F_{1-\alpha, m-1, n-1}$ |
| $H_{a}: \sigma_{1}^{2} \neq \sigma_{2}^{2}$ | either $f \geq F_{\alpha/2, m-1, n-1}$ or $f \leq F_{1-\alpha/2, m-1, n-1}$ |
:::

## Exercise 10.62 {.smaller}
...
Carry out a test at significance level .10 to see whether the population standard deviations for the two age groups are different (normal probability plots support the necessary normality assumption). Carry it out based on rejection region approach, AND p-value approach.

## Confidence Intervals

-   A CI for $\sigma_{1}/\sigma_{2}$ is based on repacing F in the probability statement:

$$
P(F_{1-\alpha/2,\nu_{1},\nu_{2}} < F < F_{\alpha/2,\nu_{1},\nu_{2}}) = 1-\alpha
$$

-   A (1-$\alpha$)100% CI for $\sigma_{1}^{2}/\sigma_{2}^{2}$ is given by:

$$
(\frac{s_{1}^{2}}{s_{2}^{2}}\frac{1}{F_{\alpha/2,m-1,n-1}},\frac{s_{1}^{2}}{s_{2}^{2}} F_{\alpha/2,n-1,m-1})
$$

-   Prove it!

## Confidence Interval for $\sigma_{1}^{2}/\sigma_{2}^{2}$

## Exercise 10.66

An article reports the following data on impregnated compressive modulus (psi $\times$ 106) when two different polymers were used to repair cracks in failed concrete.

Epoxy 1.75 2.12 2.05 1.97.  
MMA prepolymer 1.77 1.59 1.70 1.69

Obtain a 90% confidence interval for the ratio of variances.

## Summary {.smaller}

-   The F distribution is a positive, right-skewed distribution with two degrees of freedom called the numerator and denominator degrees of freedom.
-   $$
    F_{m-1,n-1} = \frac{S_{1}^{2}/\sigma_{1}^{2}}{S_{2}^{2}/\sigma_{2}^{2}}
    $$
...
$$
(\frac{s_{1}^{2}}{s_{2}^{2}}\frac{1}{F_{\alpha/2,m-1,n-1}},\frac{s_{1}^{2}}{s_{2}^{2}} F_{\alpha/2,m-1,n-1})
$$

## Practice Problems

-   Odd problems 10.61 - 10.67
