...
1.  $\mu$ = true mean drying time for a test specimen
2.  $H_{0}: \mu = 75$
3.  $H_{a}: \mu < 75$
...
7.  Since $z = -1.5 \nleq -2.33$, we fail to reject $H_{0}$ for $H_{a}$. There is no statistical evidence to conclude that the true mean drying time for a specimen is less than 75.
...
Recall from Chapter 8, a large $n$ implies that the sample standard deviation $s$ will be close to $\sigma$ for most sampes, so that the standardized variable
...
7.  Reject $H_{0}$ since $t = -2.991 \leq -2.776$. Therefore, we have enough statistical evidence to conclude that the true average viscosity of the asphalt is not 3000. The requirement is not met.
